...
static inline void buddy_block_init(buddy_block_info_t *list)
{
	list->next = list;
	list->prev = list;
}

static inline void buddy_block_add(buddy_block_info_t *list, buddy_block_info_t *node)
{
	node->next = list;
	node->prev = list->prev;
	list->prev->next = node;
...
static inline void buddy_block_remove(buddy_block_info_t *list)
{
...
static inline bool buddy_block_empty(buddy_block_info_t *list)
{
	return list->next == list;
}

static inline buddy_block_info_t *buddy_block_pop(buddy_block_info_t *list)
{
	buddy_block_info_t *front = NULL;
	if (!buddy_block_empty(list))
	{
		front = list->next;
		buddy_block_remove(front);
	}
	return front;
}
...
static void *buddy_alloc_from_level(buddy_allocator_t *allocator, unsigned long int level)
{
	...
	for (block_at_level = level; block_at_level >= 0; --block_at_level) {
		if (!buddy_block_empty(&allocator->free_blocks[block_at_level])) {
			block_ptr = buddy_block_pop(&allocator->free_blocks[block_at_level]);
			break;
		}
	}
	...
	if (block_at_level != level) {
		...
		while (block_at_level < level) {
			...
			buddy_block_add(&allocator->free_blocks[block_at_level + 1], buddy_block_ptr);
...
static void buddy_release_at_level(buddy_allocator_t *allocator, void *ptr, unsigned long int level)
{
	...
	buddy_block_add(&allocator->free_blocks[level], ptr);
}
...
void buddy_init(buddy_allocator_t *allocator, void *address, size_t size)
{
	...
	for (i = 0; i < allocator->max_level + 1; ++i)
		buddy_block_init(&allocator->free_blocks[i]);
	...
	buddy_block_add(&allocator->free_blocks[0], (buddy_block_info_t *)address);
}
...
buddy_allocator_t *buddy_create(void *address, size_t size)
{
	...
	for (int level = 0; level < initial_allocator->max_level + 1; ++level) {
		...
		buddy_block_init(&final_allocator->free_blocks[level]);

		/* For non-empty free block lists in the initial allocator, rewrite the head and tail pointers */
		if (!buddy_block_empty(&initial_allocator->free_blocks[level])) {
...
size_t buddy_largest_available(const buddy_allocator_t *allocator)
{
	...
	for (unsigned long int level = 0; level < allocator->max_level + 1; ++level)
		if (!buddy_block_empty(&allocator->free_blocks[level]))
